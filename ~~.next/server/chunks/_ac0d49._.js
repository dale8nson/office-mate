...
__turbopack_export_value__({
    '3c4dc0eea0009f6a228b1596ad87e1650c04f076': (...args)=>Promise.resolve(__turbopack_require__("[project]/app/desktop/cloud/cloud-actions.ts [app-rsc] (ecmascript)")).then((mod)=>(0, mod['getThumbnail'])(...args)),
...
const getFileParentIDs = async (fileId)=>{
    ...
    const req = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$web$2f$exports$2f$next$2d$request$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"](`https://www.googleapis.com/drive/v3/files/${fileId}?fields=*`, {
        headers: {
    ...
    if (!file) {
        console.error(new Error(`FILE NOT FOUND: fileId: ${fileId}`));
        return null;
    }
    // console.log("file.parents:", file?.parents[0] || file?.name);
    const parent = file?.parents ? file.parents[0] : null;
    return parent;
};
const getFiles = async ()=>{
    const access_token = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$headers$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["cookies"]().get('access_token')?.value;
    const req = new __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$web$2f$exports$2f$next$2d$request$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"]('https://www.googleapis.com/drive/v3/files', {
        headers: {
            Authorization: `Bearer ${access_token}`
        }
    });
...
const getThumbnail = async (fileId)=>{
    ...
    const file = await fetch(req).then((res)=>res.json()).catch((error)=>null);
...
const Page = async ()=>{
    ...
    if (!!session) {
        ...
        let filteredFiles = files.filter((file)=>!file.mimeType.includes('folder'));
        ...
        folders.forEach(async (folder, folderIndex)=>{
            ...
            const group = filteredFiles.filter(async (file)=>{
                const parentIDs = await getFileParentIDs(file.id);
                return parentIDs ? folderId === parentIDs : console.error(`File not found: ${file.name}`), false;
            });
            // console.log(`${folder.name} group:`, group.map(file => file.name));
            tabFileGroups[folder.id] = group.map((file)=>file.id);
        });
        const folderNames = Array(Object.keys(tabFileGroups).length).fill('').map(async (id)=>{
            const name = await __TURBOPACK__imported__module__$5b$project$5d2f$app$2f$desktop$2f$cloud$2f$cloud$2d$actions$2e$ts__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["getFile"](id, 'fields=name').then((res)=>res.jason());
            return name;
        });
        console.log(`folderNames:`, folderNames);
        console.log(`groupFiles:`, tabFileGroups);
        // let tabFileGroups = Array<Array<FileCardProps>>();
        // groupFiles.forEach((group, groupIndex) => {
        //   let fileGroup = Array<FileCardProps>();
        //   group.forEach(async file => {
        //     const fileName = file.name;
        //     const thumbLink = await getThumbnail(file.id);
        //     let size: Size = { width: 256, height: 256 };
        //     if (typeof thumbLink === 'string') {
        //       const ab = await fetch(thumbLink).then(res => res.arrayBuffer());
        //       if (!!ab && typeof ab !== 'number') {
        ...
        //       }
        //     }
        //     fileGroup.push({ thumbLink, fileName, size });
        //   })
        //   tabFileGroups.push(fileGroup);
        // });
        // tabFileGroups = folders.map(async (folder, folderIndex) => {
        // // console.log(`folder:`, folder);
        // const groupFiles = await filteredFiles.filter(async (file, index) => {
        //   // console.log(`groupFiles file:`, file);
        //   const f =  await getFileParentIDs(file.id).catch(err => console.log(err));
        //   // console.log(`folder: ${file.name}`)
        //   if (!!f) {
        //     return f[0] === folder?.id;
        //   } else return false;
        //   // return null;
        // });
        // // console.log(`groupFiles no. ${folderIndex}:`, groupFiles);
        // const fileCards = await groupFiles.map(async (file, fileIndex) => {
        //   const thumbLink = await getThumbnail(file.id).catch(err => null);
        // // console.log(`THUMBLINK:`, thumbLink);
        // let size: Size = { width: 256, height: 256 };
        // // console.log(`typeof thumbLink:`, typeof thumbLink);
        // if (!!thumbLink && typeof thumbLink === 'string') {
        // // console.log(`thumbLink: ${thumbLink}`);
        // const ab = await fetch(thumbLink).then(res => res.arrayBuffer()).catch(err => null);
        // // console.log(`ab:`, ab);
        // if (!!ab && typeof ab !== 'number') {
        // const buf = Buffer.from(ab);
        // // console.log(`buf:`, buf)
        ...
        //   }
        // }
        // return { thumbLink: thumbLink, fileName: file.name, size };
        // });
        // return fileCards;
        // })
        //   fileCards = filteredFiles.map(async file => {
        //   const thumbLink = await getThumbnail(file.id).catch(err => errorLog.push(<Typography variant='body1' >{err}</Typography>));
        //   // console.log(`thumbLink: ${thumbLink}`);
        //   if (thumbLink) {
        //     console.log(`THUMBLINK`);
        //     const ab = await fetch(thumbLink as string).then(res => res.arrayBuffer()).catch(err => errorLog.push(<Typography variant='body1' >{err}</Typography>));
        //     if(!!ab) {
        //     const buf = Buffer.from(ab);
        //     size = imageSize(buf);
        //     } else { size = {width: 256, height: 256}};
        //   } else {
        ...
        //   );
        // });
        // const file = await getFile(files[0].id);
        // console.log(`files[0]:`, file);
        // console.log(`cloud page files:`, files);
        // }
        // console.log(`server side tabFileGroups:`, tabFileGroups);
        // console.log(`tabFileGroups:`, tabFileGroups);
        return /*#__PURE__*/ __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$future$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"](__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$mui$2f$material$2f$Box$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"], {
            component: "main",
            sx: {
                display: 'flex',
            ...
            children: [
                ...
                /*#__PURE__*/ __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$future$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$rsc$2f$react$2d$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["jsxDEV"](__TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f40$mui$2f$material$2f$Box$2f$index$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["default"], {
                    ...
                    sx: {
                        ...
                        gridAutoColumns: 'minmax(100px, 256px)',
                        rowGap: '0.625%'
                    }
                }, void 0, false, {
                    fileName: "<[project]/app/desktop/cloud/page.tsx>",
...
//# sourceMappingURL=_ac0d49._.js.map